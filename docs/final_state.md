...
## 8. Repository layout after closeout

The three extra working directories are gone. Nothing was lost with them, but the layout the
earlier sections describe is no longer what is on disk, so it is written down here.

**The forks live as branches only.** `fork/contrastive` (`8c7a03d`) and `fork/crossattn`
(`16a050c`) were checked out as sibling worktrees at `../cognate-contrastive` and
`../cognate-crossattn` while Phase D ran. Both were fully pushed and clean, so the worktrees were
removed with `git worktree remove` and the branches left intact on `origin`. To read either fork
again, `git checkout fork/contrastive` — or re-create a worktree. The merge commits on `main`
(`be0d1c9`, `d772293`) already carry their content, so neither branch is needed to reproduce a
result; they are kept for provenance.

**`shared/` holds the derived ESM-2 caches and is gitignored.** Six `.npz` files,
3,047,941,228 bytes (2.8 GiB), previously at `../cognate-shared` and moved inside the repo now
that no sibling worktree needs to reach them. `cognate.embed.default_cache_dir()` resolves to
`<repo>/shared` unless `COGNATE_CACHE_DIR` overrides it; that function is the only place the path
is written, and every script routes through it.

The path is stated as `/Users/freddy/Documents/repos/cognate-shared` in
[`ADR/0001-frozen-contract.md`](ADR/0001-frozen-contract.md) §"one limit" and in
`data/frozen_contract_hashes.json`. Those records were deliberately **not** rewritten. They attest
to sha256 sums of files at a path on a date; editing the path inside them afterwards would make a
frozen attestation say something it did not say when it was signed. The files are the same files —
the move was verified byte-identical — and this section is the forwarding address.

**Nothing in `shared/` is backed up.** It is ignored by git, so `origin` does not have it, and it
sits in no other copy. Losing the disk loses all 2.8 GiB. That is a deliberate trade — the files
are large and regenerable — but it is only acceptable because the regeneration path is committed:

| file | regenerated by |
|---|---|
| `esm2_8M.npz`, `esm2_35M.npz` | `scripts/build_embeddings.py` (loops both models in one run) |
| `esm2_8M_vdjdb.npz`, `esm2_35M_vdjdb.npz` | `scripts/build_vdjdb_embeddings.py --model {8M,35M}` |
| `esm2_35M_train_residues.npz`, `esm2_35M_eval_residues.npz` | `scripts/build_residue_embeddings.py --model 35M --split {train,eval}` |

`scripts/fetch_data.sh` must run first; the raw inputs are gitignored too. Two real gaps remain in
that path. There is **no single orchestrator** — regenerating everything is five separate
invocations across three scripts, and `run_all.py` does not call any of them, so the sequence
exists only in this table. And regeneration reproduces the files, not the *bytes*: the recorded
sha256 sums in `data/frozen_contract_hashes.json` are of these particular artifacts, so a rebuild
on different hardware or a different `transformers` version will not re-verify against them.
`scripts/verify_residue_cache.py` checks residue-vs-pooled consistency and would still pass.
